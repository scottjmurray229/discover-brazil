...
interface Itinerary {
  title: string;
  subtitle: string;
  totalBudget?: { brl: number; usd: number };
  days: Day[];
}

const DEST_NAMES: Record<string, string> = {
  'rio-de-janeiro': 'Rio de Janeiro', 'sao-paulo': 'S\u00e3o Paulo', 'salvador': 'Salvador',
  'florianopolis': 'Florian\u00f3polis', 'iguazu-falls': 'Iguazu Falls', 'brasilia': 'Bras\u00edlia',
  'manaus': 'Manaus', 'recife': 'Recife', 'paraty': 'Paraty',
  'ouro-preto': 'Ouro Preto', 'jericoacoara': 'Jericoacoara', 'fernando-de-noronha': 'Fernando de Noronha', 'bonito': 'Bonito',
};
...
function buildAffiliateUrl(type: string | null | undefined, destination: string, description: string, tp?: TripParams): string {
  ...
  if (type === 'hotel') {
    ...
    const query = hotelName ? hotelName + ' ' + destName : destName + ' Brazil';
    let url = `https://www.booking.com/searchresults.html?ss=${enc(query)}&aid=2778866&label=discoverbrazil`;
  ...
  if (type === 'tour') {
    ...
    return `https://www.getyourguide.com/s/?q=${enc(query)}&partner_id=IVN6IQ3&cmp=discoverbrazil`;
  }
  if (type === 'transport') {
    return `https://www.getyourguide.com/s/?q=${enc(destName + ' transport transfer')}&partner_id=IVN6IQ3&cmp=discoverbrazil`;
  }
  return `https://www.getyourguide.com/s/?q=${enc(destName)}&partner_id=IVN6IQ3&cmp=discoverbrazil`;
}
...
function buildHotelsComUrl(destination: string, description: string): string {
  ...
  const landingPage = encodeURIComponent(`https://www.hotels.com/Hotel-Search?destination=${encodeURIComponent(query + ' Brazil')}`);
...
function buildItineraryEmail(it: Itinerary, tp?: TripParams): string {
  ...
  for (const day of it.days) {
    ...
    for (const item of day.items) {
      ...
      let priceStr = '';
      if (item.priceBrl) {
        dayBrl += item.priceBrl;
        if (item.priceUsd) dayUsd += item.priceUsd;
        priceStr = ` — R$${item.priceBrl.toLocaleString()}`;
    ...
    const subtotalHtml = dayBrl > 0 ? `<tr><td colspan="2" style="text-align:right;font-size:12px;font-weight:600;color:#1A2332;padding-top:8px;border-top:1px solid #EBE4D8;">Day total: R$${dayBrl.toLocaleString()} (~$${dayUsd} USD)</td></tr>` : '';
  ...
  const budgetHtml = it.totalBudget ? `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#E8F4F5;border-radius:10px;padding:12px 20px;margin-bottom:20px;">
      ...
      <span style="font-size:16px;font-weight:800;color:#1A2332;">R$${it.totalBudget.brl?.toLocaleString() || '—'} <span style="font-size:13px;font-weight:400;color:#0D7377;">(~$${it.totalBudget.usd?.toLocaleString() || '—'} USD)</span></span>
    </div>` : '';
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:linear-gradient(135deg,#0D7377,#14B8A6);border-radius:16px 16px 0 0;padding:28px 24px;text-align:center;">
      <h1 style="color:white;font-size:22px;margin:0 0 6px;">Your Brazil Itinerary</h1>
    ...
    <div style="background:#F5F0E8;border-radius:0 0 16px 16px;padding:24px 20px;">
      ...
      <div style="text-align:center;padding:24px 0 16px;">
        <a href="https://discoverbrazil.info/plan/" style="display:inline-block;background:#E8654A;color:white;padding:14px 32px;border-radius:100px;text-decoration:none;font-weight:700;font-size:15px;">Generate Another Itinerary</a>
      </div>

      <p style="font-size:12px;color:#94A3B8;text-align:center;line-height:1.5;margin:0;">
        Generated by <a href="https://discoverbrazil.info/" style="color:#0D7377;text-decoration:none;">Discover Brazil</a> AI Trip Planner<br>
        Real prices &middot; Real experience &middot; Your Brazil travel guide
      </p>
    </div>

    <!-- Footer -->
    <p style="color:#94A3B8;font-size:11px;text-align:center;margin:16px 0 0;line-height:1.5;">
      <a href="https://discoverbrazil.info/legal/privacy/" style="color:#94A3B8;">Privacy Policy</a> &middot;
      <a href="https://discoverbrazil.info/legal/affiliate-disclosure/" style="color:#94A3B8;">Affiliate Disclosure</a>
    </p>
  </div>
</body>
</html>`;
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Discover Brazil <hello@discoverbrazil.info>',
        to: [email],
        subject: `Your Brazil Itinerary: ${it.title}`,
        html,
      }),
    });
