...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['rio-de-janeiro', 'florianopolis', 'fernando-de-noronha'],
    'nightlife': ['rio-de-janeiro', 'sao-paulo', 'florianopolis'],
    'architecture': ['brasilia', 'ouro-preto', 'sao-paulo'],
    'history': ['ouro-preto', 'salvador', 'rio-de-janeiro'],
    'nature': ['manaus', 'bonito', 'iguazu-falls'],
    'food': ['sao-paulo', 'salvador', 'rio-de-janeiro'],
    'wine': ['sao-paulo', 'florianopolis', 'rio-de-janeiro'],
    'budget': ['salvador', 'recife', 'manaus'],
    'backpacker': ['rio-de-janeiro', 'florianopolis', 'jericoacoara'],
    'carnival': ['rio-de-janeiro', 'salvador', 'recife'],
    'culture': ['salvador', 'rio-de-janeiro', 'ouro-preto'],
    'hiking': ['rio-de-janeiro', 'bonito', 'manaus'],
    'amazon': ['manaus'],
    'diving': ['fernando-de-noronha', 'bonito'],
    'romantic': ['fernando-de-noronha', 'paraty', 'rio-de-janeiro'],
    'adventure': ['bonito', 'manaus', 'jericoacoara'],
    'southeast': ['rio-de-janeiro', 'sao-paulo', 'paraty', 'ouro-preto'],
    'northeast': ['salvador', 'recife', 'jericoacoara', 'fernando-de-noronha'],
    'south': ['florianopolis', 'iguazu-falls', 'bonito'],
    'central-north': ['brasilia', 'manaus'],
  };
  ...
  return ['rio-de-janeiro', 'salvador', 'sao-paulo'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Brazil AI Trip Planner. You are an expert on travel across Brazil — from the beaches of Rio de Janeiro to the Amazon rainforest of Manaus.

RULES:
- All prices in BOTH BRL (R$) and USD ($). Use rate: R$1 = $0.20 USD
- Include specific restaurant names, hotel recommendations, and transport details (domestic flights, buses, transfers)
...
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-rio-de-janeiro"
- Brazil uses a single currency: the Brazilian Real (BRL/R$). Express all prices in BRL for consistency
...
{
  "title": "string — trip title like '7-Day Brazil Adventure'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "brl": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Rio de Janeiro'",
      "destination": "string — slug like 'rio-de-janeiro'",
      "items": [
        {
          "time": "string — like 'Morning' or '5:30 AM'",
          "description": "string — what to do, with personality",
          "priceBrl": number_or_null,
          "priceUsd": number_or_null,
          "category": "transport|accommodation|activity|food|ferry",
          "affiliateType": "hotel|tour|transport|null",
          "affiliateSlotId": "string_or_null",
          "locationName": "string — specific place name for map pin, e.g. 'Charles Bridge', 'Széchenyi Thermal Bath', 'Old Town Square'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Brazil trip:\n${parts.join('\n')}`;
